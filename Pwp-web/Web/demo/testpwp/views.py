...
def get_specifics(key):
	...
	for i in result_Pla.values():
	  ...
	  # count_individual_W.append([i["detectionClassesTesting"]["Wrapper"]])
	  # W_individual_cont.append(i["detectionScoreTesting"]["Wrapper"])
	  # count_wrap += i["detectionClassesTesting"]["Wrapper"]
	  # total_wrapper += sum(i["detectionScoreTesting"]["Wrapper"])
...
def get_specifics_brands(key):
	...
	for i in result_Bra.values():
	  count_individual.append(i["detectionClassesTesting"][key])
	  individual_contri.append(i["detectionScoreTesting"][key])
	  count_overall += i["detectionClassesTesting"][key]
	  total_contri += sum(i["detectionScoreTesting"][key])
	  # count_individual_W.append([i["detectionClassesTesting"]["Wrapper"]])
	  # W_individual_cont.append(i["detectionScoreTesting"]["Wrapper"])
	  # count_wrap += i["detectionClassesTesting"]["Wrapper"]
	  # total_wrapper += sum(i["detectionScoreTesting"]["Wrapper"])
...
def modal(request):
    import os
    path = os.chdir("/home/milan/Desktop/pwp21 (copy)/DeepBlue_1/Pwp-web/Web/demo/testpwp/static/image/image_plastic/")
    files = os.listdir(path)
    # print()
    image = ["image/image_plastic/"+i for i in files if i.endswith('.jpg')]
    print(image)
    context={'image':image}
...
def brand(request):
    import os
    path = os.chdir("/home/milan/Desktop/pwp21 (copy)/DeepBlue_1/Pwp-web/Web/demo/testpwp/static/image/image_brand/")
    files = os.listdir(path)
    # print()
    image = ["image/image_brand/"+i for i in files if i.endswith('.jpg')]
    print(image)
    context={'image':image}
